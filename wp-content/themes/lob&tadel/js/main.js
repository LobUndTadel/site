...
!function() {
	...
	var controller = {
		...
		formSubmit : function( event ){
			...
			this.$mailHolder.removeClass('error');

			// strip @hs-furtwangen.de if #userBlicktsNicht
			val = val.replace(/\@hs\-furtwangen\.de/, '');

			if( /\@(.+)\.(.+)/.test(val)  ){
				this.$mailHolder.addClass('error');
				return;
			}

			// append @hs-furtwangen.de again
			val += '@hs-furtwangen.de';
			...
			$.ajax({
				...
				success : $.proxy(function( respText ){
					if( respText == 'Wrong email'){
						...
						this.$el.find('.newsletter-subscription').addClass('hidden');
					}
				}, this)
			});
		},
...
!function(){
	...
	FX.canvas = document.getElementById('particle');
